...
def save_rule(rule, path, length, min_support_num, min_support, min_conf):  # 保存结果到txt文件
	with open(path, "w") as f:
		#{:<10}左对齐，补齐10位，{：.2%}保留两位小数加上百分号
		strs = "data_length : {:<10}  min_support_num : {} \n" \
			   "min_support : {:<10.2%}  min_conf : {:.2%} \n\n"\
			.format(length,min_support_num,min_support,min_conf)
		f.write(strs)
...
class Fp_growth():
	...
	def generate_L(self, data_set, min_support):
		...
		# print("generate_L's",headerTable)
